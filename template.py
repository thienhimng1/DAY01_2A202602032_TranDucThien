...
# Nạp OPENAI_API_KEY (và OPENAI_BASE_URL nếu dùng OpenRouter/NVIDIA NIM)
# từ file .env — copy .env.example thành .env và dán key vào.
load_dotenv()
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    def trunc(text: str, length: int = 40) -> str:
        text = str(text).replace("\n", " ")
        return text if len(text) <= length else text[:length - 3] + "..."

    header = (f"{'Prompt':<40} | {'GPT-4o Response':<40} | "
              f"{'Mini Response':<40} | {'GPT-4o Latency':<15} | "
              f"{'Mini Latency':<15}")
